function ReviewCard({comment,name}) {
  ...
  return (
    <div className="shadow-xl flex flex-col justify-center items-center text-white py-5 px-10 w-50 gap-2 lg:flex-row">
      <div>
        <img src="src/assets/icons/profile-pic.svg" alt="profile-pic" 
          className="w-32"
        />
      </div>
      <div className="flex flex-col lg:flex-col-reverse justify-center items-center lg:items-start lg:gap-6 gap-1 w-full">
        <p className="w-60 lg:w-full font-secondary italic">
          "{comment}"
        </p>
        <h3 className="font-bold tracking-wide font-secondary">
          {name}
        </h3>
      </div>
